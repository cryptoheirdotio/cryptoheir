...
cd cryptoheir-go
go mod tidy
go build -o cryptoheir ./cmd/cryptoheir
```
...
```bash
go install ./cmd/cryptoheir
```

### Build Static Binary (for distribution)

```bash
CGO_ENABLED=0 go build -ldflags="-s -w" -o cryptoheir ./cmd/cryptoheir
```
...
│   ├── contract/contract.go     # ABI encoding
...
```bash
# Linux
GOOS=linux GOARCH=amd64 go build -o cryptoheir-linux-amd64 ./cmd/cryptoheir

# macOS
GOOS=darwin GOARCH=amd64 go build -o cryptoheir-darwin-amd64 ./cmd/cryptoheir
GOOS=darwin GOARCH=arm64 go build -o cryptoheir-darwin-arm64 ./cmd/cryptoheir

# Windows
GOOS=windows GOARCH=amd64 go build -o cryptoheir-windows-amd64.exe ./cmd/cryptoheir
```
...
→ Run `cd ../foundry && forge build` to compile contracts first
